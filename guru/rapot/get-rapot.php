...
$tglCetak = 'Denpasar, ' . getTanggal();
$waliMurid = $dt['nama_ayah'];
$kelas = 'Guru Kelas ' . $kls['nama_kelas'];
$waliKelas = $kls['nama'];
$nipWali = $kls['wali_kelas'];
...
// $pdf->SetX(4);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(190, 0, 'RAPOR DAN PROFIL PESERTA DIDIK', 0, 0, 'C');
$pdf->ln(10);
// $pdf->SetX(4);
...
while ($mpl = mysqli_fetch_array($query)) {
    ...
    if ($mpl['mulok'] == 0) {
        ...
        $pdf->SetXY($xPos + 30, $yPos);
        $pdf->Cell(10, 55, round($mpl['rata']), 'LB', 0, 'C');
        $pdf->Cell(15, 55, getPredikat(round($mpl['rata'])), 'LB', 0, 'C');
        ...
        if ($ml == 0) {
            $pdf->SetFont('Arial', 'B', 10);
            $pdf->Cell(190, 10, 'Muatan Lokal', 1, 0, 'L');
            $pdf->ln(10);
            $ml = 1;
        }
